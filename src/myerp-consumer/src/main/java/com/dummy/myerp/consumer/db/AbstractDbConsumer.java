...
// ==================== Attributs Static ====================
     ...
     */
    protected <T> T queryGetSequenceValuePostgreSQL() {

        JdbcTemplate vJdbcTemplate = new JdbcTemplate(getDataSource(DataSourcesEnum.MYERP));

        return vJdbcTemplate.queryForObject("SELECT last_value FROM myerp.ecriture_comptable_id_seq", (Class<T>) Integer.class);
    }
        ...
        for (DataSourcesEnum vDataSourceId : vDataSourceIds) {
            ...
            if (vDataSource == null) {
                if (!pMapDataSource.containsKey(vDataSourceId)) {
                    LOGGER.error(String.format("La DataSource %s n'a pas été initialisée !", vDataSourceId));
                }
            } else {
